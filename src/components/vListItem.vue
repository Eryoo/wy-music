<template>
    <div class="items" :class="{'dis':item.data.st == '-200'}" @click="getDatailsSong(item)">
        <div class="no">{{item.itemIndex + 1}}</div>
        <div class="info">
            <div class="name">{{item.data.name}}</div>
            <div class="ar">
                <span>{{item.data.st == '-200'?'无音源':''}}</span>
                {{ar.toString()}}</div>
        </div>
        <div class="play">
...
function getDatailsSong (item) {
    if(item.data.st == '-200') return
